...
const SecretWordGameComponent = ({ quiz, onComplete }: { quiz: Quiz, onComplete: () => void }) => {
  const secretWord = quiz.answer?.toUpperCase() || ''; const [guessedLetters, setGuessedLetters] = useState<Set<string>>(new Set()); const [mistakes, setMistakes] = useState(0); const maxLives = 6;
  const handleGuess = (letter: string) => { if (guessedLetters.has(letter) || mistakes >= maxLives) return; const newGuessed = new Set(guessedLetters); newGuessed.add(letter); setGuessedLetters(newGuessed); if (!secretWord.includes(letter)) { setMistakes(prev => prev + 1); soundService.playPop(); } else { soundService.playCoin(); } };
  useEffect(() => { const isWin = secretWord.split('').every(char => guessedLetters.has(char)); if (isWin && secretWord.length > 0) { setTimeout(onComplete, 500); } }, [guessedLetters, secretWord]);
  useEffect(() => { if (mistakes >= maxLives) { alert(`¡Oh no! Se acabaron los intentos. La palabra era: ${secretWord}`); } }, [mistakes]);
  const alphabet = "ABCDEFGHIJKLMNÑOPQRSTUVWXYZ".split('');
  return ( <div className="space-y-6"> <div className="flex justify-center gap-1 mb-4"> {[...Array(maxLives)].map((_, i) => ( <Heart key={i} size={24} className={`${i < (maxLives - mistakes) ? 'text-pink-500 fill-pink-500 animate-pulse' : 'text-slate-200 fill-slate-100'}`} /> ))} </div> <div className="flex flex-wrap justify-center gap-2 min-h-[60px] p-4 bg-pink-50 rounded-2xl border-2 border-pink-100"> {secretWord.split('').map((char, index) => ( <div key={index} className="w-10 h-12 flex items-center justify-center border-b-4 border-pink-400 font-black text-xl text-pink-700"> {guessedLetters.has(char) ? char : ''} </div> ))} </div> <div className="flex flex-wrap gap-1.5 justify-center"> {alphabet.map(letter => { const isGuessed = guessedLetters.has(letter); const isCorrect = secretWord.includes(letter); let btnClass = "bg-white border-slate-200 text-slate-700 hover:bg-slate-50"; if (isGuessed) { btnClass = isCorrect ? "bg-emerald-500 border-emerald-700 text-white" : "bg-slate-200 border-slate-300 text-slate-400 opacity-50"; } return ( <button key={letter} onClick={() => handleGuess(letter)} disabled={isGuessed || mistakes >= maxLives} className={`w-9 h-10 rounded-lg font-bold border-b-4 active:border-b-0 active:translate-y-1 transition-all text-sm ${btnClass}`} > {letter} </button> ) })} </div> </div> );
};
