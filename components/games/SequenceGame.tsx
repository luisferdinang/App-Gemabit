...
export const SequenceGame: React.FC<SequenceGameProps> = ({ quiz, onComplete, onFail }) => {
  ...
  const handleSelect = (item: { id: string, text: string }) => {
    ...
    if (item.text === expectedText) {
      ...
      // Incorrect - FAIL IMMEDIATELY
      // soundService.playPop() is handled by onFail usually, but ok to play here if onFail doesn't (StudentView handles it)
      // But let's let onFail handle the sound/alert.
      onFail();
    }
  };
